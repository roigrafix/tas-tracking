 ...
 * Any visitor can look up a Tracking ID:
 *  - Public shipments  → full details shown
 *  - Restricted ones   → "Sign in to check your access" prompt
 *  - Not found         → friendly not-found state
 *
 * reCAPTCHA is required before every search to prevent bot abuse.
 */
...
export default function PublicTrack({ onGoToLogin }) {
  ...
  const [captchaKey, setCaptchaKey]           = useState(0); // Increment to reset reCAPTCHA
  const [showCaptchaWarn, setShowCaptchaWarn] = useState(false);

  // ── Handlers ────────────────────────────────────────────────────────────────

  const handleCaptchaVerify = (token) => {
    // token is a string when verified, null when expired/error
  ...
  const handleSearch = (e) => {
    ...
    // reCAPTCHA gate — must verify before searching
    if (!captchaVerified) {
    ...
    setTimeout(() => {
      // Public track: pass null for userId — only public shipments will be returned
      const res = db.getShipmentByTrackingId(id, null);
      setResult(res);
      setSearched(true);
      setLoading(false);

      // Reset reCAPTCHA after each search (security best practice)
    ...
    return (
      <div style={{ animation: 'fadeSlideIn 400ms ease' }}>
        ...
        <div className="card mb-6" style={{ padding: '1.75rem' }}>
          <div className="flex items-center justify-between" style={{ flexWrap: 'wrap', gap: '1rem' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.375rem' }}>
                <span style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#4a5a72' }}>
                  Tracking ID
                </span>
                <span style={{ fontSize: '1.375rem', fontWeight: 800, fontFamily: 'monospace', letterSpacing: '0.06em', color: '#f0f6ff' }}>
                  {shipment.id}
                </span>
          ...
          <div style={{ fontSize: '0.7rem', color: '#4a5a72', fontWeight: 600, textAlign: 'right' }}>
  ...
  return (
    <div className="app-root">
      ...
      <div className="pub-hero">
        <div className="pub-hero-inner">
          ...
          <h1 className="pub-title">
            Track Your Parcel
            <br />
            <span className="pub-title-accent">Instantly</span>
          </h1>

          <p className="pub-subtitle">
            Enter your TAS Tracking ID below to get real-time updates on your shipment —
            no sign-in needed for public parcels.
          </p>
            ...
            <form onSubmit={handleSearch} noValidate>
              ...
              <div className="pub-search-row">
                ...
                <button
                  id="public-track-btn"
                  type="submit"
                  className="btn btn--primary btn--lg pub-search-btn"
                  disabled={loading}
                >
                  {loading
              ...
              <div className="pub-captcha-wrap">
                ...
                {showCaptchaWarn && (
                  <div className="captcha-warn">
                    ⚠️ Please complete the reCAPTCHA verification before searching.
                  </div>
                )}
              </div>
            </form>
      ...
      <div className="pub-results page-content" style={{ position: 'relative', zIndex: 1 }}>
        ...
        {searched && result && !result.found && (
          <div className="access-denied">
            ...
            <p className="text-secondary" style={{ maxWidth: 420, textAlign: 'center' }}>
              No shipment matching <strong>{trackingInput}</strong> was found.
        ...
        {searched && result && result.found && !result.access && (
          <div className="access-denied">
            ...
            <p className="text-secondary" style={{ maxWidth: 460, textAlign: 'center' }}>
              Shipment <strong>{trackingInput}</strong> exists but is restricted to authorised users.
              Sign in to check whether your account has been granted access.
            </p>
        ...
        {/* Found with access: full shipment view */}
      ...
      <div className="pub-features-strip">
        <div className="pub-feature">
          <span className="pub-feature-icon">⚡</span>
          <div>
            <div className="pub-feature-title">Real-time Updates</div>
            <div className="pub-feature-sub">Live status changes as they happen</div>
          </div>
        </div>
        <div className="pub-feature-divider" />
        <div className="pub-feature">
          <span className="pub-feature-icon">🌍</span>
          <div>
            <div className="pub-feature-title">Global Coverage</div>
            <div className="pub-feature-sub">Air, sea, and land freight worldwide</div>
          </div>
        </div>
        <div className="pub-feature-divider" />
        <div className="pub-feature">
          <span className="pub-feature-icon">🔐</span>
          <div>
            <div className="pub-feature-title">Secure &amp; Private</div>
            <div className="pub-feature-sub">RBAC-protected for sensitive cargo</div>
          </div>
        </div>
        <div className="pub-feature-divider" />
        <div className="pub-feature">
          <span className="pub-feature-icon">📱</span>
          <div>
            <div className="pub-feature-title">Any Device</div>
            <div className="pub-feature-sub">Mobile, tablet, and desktop ready</div>
          </div>
        </div>
      </div>
      ...
      <style>{`
        ...
        .public-grid-overlay {
          ...
          pointer-events: none;
          background-image:
            linear-gradient(rgba(99,162,255,0.03) 1px, transparent 1px),
            linear-gradient(90deg, rgba(99,162,255,0.03) 1px, transparent 1px);
        ...
        .pub-features-strip {
          ...
          gap: 0;
          padding: 2rem 2rem;
          border-top: 1px solid rgba(99,162,255,0.08);
          border-bottom: 1px solid rgba(99,162,255,0.08);
        ...
        @media (max-width: 640px) {
          ...
          .pub-features-strip { flex-direction: column; gap: 0; }
          .pub-feature-divider { width: 80%; height: 1px; margin: 0 auto; }
